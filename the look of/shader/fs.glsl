...
vec2 mapFlux(vec3 p, float time, vec3 pos) { 
    ...
    // piece 3: fade? - yes, this is the final 'flow'. maybe multiple layers.
    {
        float NoiseScale = 0.21723391;
        // pos -= vec3(0,4.5,0);
        // p.y = max(p.y,5.0);
        // vec3 b = 0.25*vec3(1.0, 1.0, 1.0);
        // vec3 q = abs(p) - b; // distances in separate dims
        // float ss = (min(smoothstep(1.0, 0.0, q.x), min(smoothstep(0.5, 0.0, q.y), smoothstep(1.0, 0.0, q.z))));
        float ss = sdBox(p + pos, vec3(0.8, 1.25, 0.8)) - 0.1;
        float NoiseIsoline = 0.00 + 0.25929419 * (smoothstep(1.0, 0.0, ss) * smoothstep(4.58, 4.575, p.y)); //smoothstep(5.5, 1.0, q.y));
        // float NoiseIsoline = 0.06 + 0.329419 * ss;
        ...
        float noise = NoiseScale * (fbm(p + 0.322*fbm( p + 0.125*vec3(1.99821,2.003,1.552)*time )) - NoiseIsoline);
        // p.x -= noise;

        // float d = sdCapsule(p, vec3(-2.0, 0.0, 0.0), vec3(-2.0,1.0,0.0)).x - ( 0.5 + 0.5 * ( sizeSignal(time)) ); // capsule // two capsules alternating?
        res = opU(res, vec2(noise, 30.0));
    }

    {
        float NoiseScale = 1.29391;
        // pos -= vec3(0,4.5,0);
        // p.y = max(p.y,5.0);
        // vec3 b = 0.25*vec3(1.0, 1.0, 1.0);
        // vec3 q = abs(p) - b; // distances in separate dims
        // float ss = (min(smoothstep(1.0, 0.0, q.x), min(smoothstep(0.5, 0.0, q.y), smoothstep(1.0, 0.0, q.z))));
        float ss = sdBox(p3 + pos, vec3(0.8, 1.65, 1.2)) - 0.1;
        float NoiseIsoline = 0.00 + 0.4282419 * (smoothstep(1.0, 0.0, ss) * smoothstep(4.58, 4.575, p3.y)); //smoothstep(5.5, 1.0, q.y));
        // float NoiseIsoline = 0.06 + 0.329419 * ss;
        // float NoiseIsoline = 0.419 * (smoothstep(6.0, 2.0, p.y));
        // float NoiseIsoline = 0.419;
        p3 = (p3 / NoiseScale) + time * vec3(0,0.05,0);// - time;// * vec3(0,0.1,0);
        // float noise = NoiseScale * (fbm(p) - NoiseIsoline);

        float noise = NoiseScale * (fbm(p3 + 0.10*fbm( p3 + 0.32*vec3(1.19821,0.66703,1.252)*time )) - NoiseIsoline);
        // p.x -= noise;

        // float d = sdCapsule(p, vec3(-2.0, 0.0, 0.0), vec3(-2.0,1.0,0.0)).x - ( 0.5 + 0.5 * ( sizeSignal(time)) ); // capsule // two capsules alternating?
        res = opSmoothU(res, vec2(noise, 30.0), 0.5);
    }

    {
        float d = sdCapsule(p2 + vec3(0.0, -4.575, 10.0), vec3(-1.5,0,0), vec3(1.5,0,0)).x;
        res = opU(res, vec2(d, 40.0));
    }

    return res;

    // piece 1: globe-y thing
    // {
    //     vec3 p2 = p + vec3(0.0, -3.5, 10.0);
    //     l = 2.0*vec3(3,2,2);
    //     // l = vec3(10);
    //     vec3 id = clamp(round(p2/extents),-l,l);
    //     vec3 stu = fract(p2 / extents);  
    //     vec3 q = p2-extents*id;

    //     float minExtent = min(extents.x, min(extents.y, extents.z));
    //     vec3 n = N13(id.x*12.45+id.y*33.654+id.z*76.5);

    //     float fade = Saw(.025, fract(time+n.z));

    //     time *= 1.0;
    //     // float noise = 45.*clamp(fbm( time*n + p2*2.0 ), -1.0, 1.0);
    //     // // float noiseAmount = 1.0 - smoothstep(0.0, 3.05, p.y);
    //     // q.x -= 0.015 * noise;//* noiseAmount;

    //     res = vec2( sdfSphere(q, fade*(n.x*0.1)), 15.0 );
    // }

    // {
    //     float NoiseScale = 2.91;
    //     float NoiseIsoline = 0.319;
    //     p = p / NoiseScale + time * vec3(0,0.4,0);
    //     float noise = NoiseScale * (fbm( p ) - NoiseIsoline);
    //     // p.x -= noise;

    //     // float d = sdCapsule(p, vec3(-2.0, 0.0, 0.0), vec3(-2.0,1.0,0.0)).x - ( 0.5 + 0.5 * ( sizeSignal(time)) ); // capsule // two capsules alternating?
    //     return opSmoothU(res, vec2(noise, 30.0), 0.43);
    // }
...
vec2 map (vec3 p, float time) { 
    ...
    // p.z = max(-p.z, 40.0);
        ...
        vec3 pos = vec3(0.0, -4.0, 10.0);
        float dFlux = opIntersection(mapFlux(p, time, pos).x, sdfSphere(p + pos, 4.0));
        res = opU(res, vec2(dFlux, 30.0));

    // vec3 pos = vec3(0.0, -0.5, 10.0);
    // res = opU(res, mapFlux(p - vec3(0,3.5,-15), time, pos));
