...
// map x,y,z... (organization)
// use sdfs in here - main world map
vec2 map (vec3 p, float time) { 
    vec2 res = vec2(1e10, 0.0);

    res = vec2(sdfSphere(p + vec3(0.0, -2.0 - 0.5 * cos(time * 0.5), 15.0), 0.75), 15.0);
...
vec3 render(in vec3 ro, in vec3 rd, in vec3 rdx, in vec3 rdy, float time) { 
    ...
    if ( m > -1.0 && m < 5.0 ) { 
        vec3 nor = vec3(0,1,0);
        float shadow = calcSoftshadow(pos, sunLig, 0.1, 100.0, 4.0, time);

        col = col * (1.0 - smoothstep(3.6, 4.5, length(pos + vec3(0.0, 0.0, 15.0))));
        col = 0.5 * col + 0.5 * col * shadow;
    }
    ...
    if ( m > 5.0 ) { 
        ...
        float shadow = calcSoftshadow(pos, sunLig, 0.1, 100.0, 16.0, time);
...
void main() {
    // camera
    vec3 ro = vec3( 0.0, 4.0, 1.0);
    vec3 ta = vec3( 0.0, 3.8 + 0.05 * sin(0.15 * uTime), 0.0);
