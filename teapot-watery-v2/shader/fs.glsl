...
vec2 mapPot (vec3 p, float time) { 
    ...
    // NoiseScale = 0.914;
        ...
        // ss = 1.0;
    float NoiseIsoline = (0.04 + 0.123839919) * (smoothstep(3.9*ss, 2.2*ss, length(vec3(0,-0.5,0) + p)));
...
vec2 mapOutlineBox (vec3 p, float time) { 
    ...
    // rotate xy then xz
    vec3 new_p = p + vec3(0.0, -2.58, 20.0);
    ...
    float dToBound = sdfSphere(new_p, 2.80);
...
vec2 mapBoxScene (vec3 p, float time) { 
    ...
    p.x -= 0.3;
    p.x += 0.6*fbm(p*0.25 + 0.034*time*fbm(p*0.111));
    p.y -= 0.8*fbm(p*0.405 + 0.02*time);
    p.y += 0.4;
...
// this 2d noise relies on a noise texture
// can either use 3d noise with constant z, find a 2d noise, or use vnoise (octaved or not)
// try 3d noise first
float noise (in vec2 x ){return noise(vec3(x, 1.0));}
...
// based on texture coordinates -- p
vec4 wateryColor( in vec2 p )
{
    // get position
	// vec2 p = vec2(1.0);
    ...
    // rotate space based on p
    int z = 3;
	float rb = fbm(p*0.5+uTime*.13)*(0.9);
	rb = sqrt(rb);
	...
	#else
	p *= makem2(rb*.4+atan(p.y,p.x)*2.);
	#endif
    }
	
    // now then get a color
	//coloring
    float rz = fbm(fbm(p*0.5 + uTime*0.03) + p*1.22-uTime*.12);
	rz *= 0.5*dot(bp*5.,bp)+.3;
	rz *= sin(p.x*.0941+uTime*0.34) + tan(p.y*0.1963-uTime*0.2222);
	vec3 col = vec3(.04,0.07,0.45)/(.1-rz);
    // vec2 q = p*.2 - uTime*.35;
	// float rz = fbm(q);
    // vec3 nor = normalize( vec3( fbm(q+p.x) - rz, p.x, fbm(q+p.y) - rz ) );
	// rz *= dot(bp*5.,bp)+.5;
	// // rz += 3.0*sin(p.x*.5+uTime*4.)*1.5;
    // rz *= sin(p.x*.5+uTime*0.23)*1.5;
	// vec3 col = vec3(.04,0.07,0.45)/(.1-rz);
    
    // vec3 lig = normalize( vec3( 0.5, -0.1, 0.9 ) );
...
vec3 render(in vec3 ro, in vec3 rd, in vec3 rdx, in vec3 rdy, float time) { 
    ...
    if (m > -1.0) { 
        ...
        vec3 wateryColor = wateryColor(new_ro.xy*3.0).xyz;

        vec3 nor = calcNormal(new_ro, time);
        // // we hit something 
        vec3 lightOne = normalize(vec3(0.2, -0.5, 0.23));
        vec3 lightTwo = normalize(vec3(-0.2, 0.5, 0.23));

        vec3 col = 0.05*vec3(0.2);
        vec3 clrOne = palette(length(0.35*new_ro.xyz + time*0.004*vec3(-0.6,1,0.22)), vec3(0.5), vec3(0.5),	vec3(1.0, 0.7, 0.4), vec3(0.00, 0.15, 0.2));
        // vec3 clrTwo = palette(length(pos.xyz*0.4 + time*0.00888*vec3(0.33,.111,2.22)), vec3(0.5), vec3(0.5), vec3(1.0), vec3(0.00, 0.1, 0.2));
        vec3 clrThree = palette(length(new_ro.z*0.2 + time*0.00132), vec3(0.5, 0.5, 0.5), vec3(0.5, 0.5, 0.5), vec3(1.0, 1.0, 1.0), vec3(0.30, 0.20, 0.20));
        // // normal --
        ...
        col += (0.255*clrOne + 0.1*sunsetClr + 0.01 * clrThree) * 0.91 * clamp(dot(lightTwo,nor), 0.0, 1.0);
        col += (0.15*clrOne + 0.75*clrThree * 0.0623 + 0.1*sunsetClr) * clamp(dot(lightOne,nor), 0.0, 1.0);
        col *= vec3(0.3, 0.25, 0.6);
        // col = 0.5 * col + wateryColor;
            
        if (m < 1.0) { 
            // didn't hit anything -- should just return sphere coloring
            // return vec3(0.2);
            return clamp(col*0.2, 0.0, 1.0);
            
        } 
        else {
            // we hit something -- render watery colors in teapot
            return clamp(0.8*wateryColor + 0.9*col*wateryColor + 0.15*col, 0.0, 1.0);
        } 
