...
float currentNoiseScalar(float time) { 
...
vec2 mapPot (vec3 p, float time) { 
    ...
    // ver 1: full pot
    {
        ...
        // if (signal > 0.00001) { 
            ...
            float currentNoiseScalar = currentNoiseScalar(time);
            NoiseScale = 1.0 * currentNoiseScalar;
            ...
            res = opSmoothU(res, vec2(noise, 40.0), 0.001 + signal*(0.81) + 0.2 * smoothstep(0.0, 7.0, uTime)); // return with metal texture
...
vec2 raycast (in vec3 ro, in vec3 rd, float time){
    ...
    for( int i = 0; i < 820 && t < tmax; i++) {
        ...
        t += h.x * (1.0 - 0.3 * smoothstep(1.0, 0.15, currentNoiseScalar(time)));
    }
...
{
    vec3 col = 0.99*vec3(.948,.9529,.9373);
...
vec3 getWaterNormal ( vec3 pos, float time ) { 
    ...
    float bump_scale = 3.0 - smoothstep(0.0, BUMP_DISTANCE, abs(pos.x));

    vec3 normal = vec3(0,0.5,0);
    float eps = 0.001;
    vec2 dx = vec2(eps, 0.0);
    vec2 dy = vec2(0.0, eps);
    normal.x = -bump_scale * 0.25 * (waterMap( pos.xy + dx, time ) - waterMap( pos.xy - dx, time )) / ( 2.0 * eps );
    normal.z = -bump_scale * 0.25 * (waterMap( pos.xy + dy, time ) - waterMap( pos.xy - dy, time )) / ( 2.0 * eps );
    normal = normalize(normal);
    return normal;
}

vec3 waterColor (in vec3 pos, vec3 rd, vec3 sunLig, float time) {
    vec3 texCol = 0.3 * texture(uWaterTexture, (1. / pow(2., 0.5)) * (vec2(0.7,0.3) + pos.yx * 2.9)).rgb;
    texCol += 0.4 * texture(uWaterTexture, (1. / pow(2., 0.5)) * (vec2(0.233,0.233) + pos.yx * 0.45)).rgb;
    texCol += 0.2 * texture(uWaterTexture, m2 * pos.xy * 0.05).rgb;
    texCol += 0.1 * texture(uWaterTexture, m2 * vec2(0.24,0.74) + pos.xy * 0.003).rgb;
    texCol *= 0.5 * 0.5;

    float TEXTURE_DISTANCE = 10.0;
    float texture_scale = 1.0;

    vec3 textureBlue = 0.5 * 0.5 * texture(uWaterTexture, vec2(0.5,0.5)).rgb;
    //vec3 col = 

    vec3 water = vec3(151.0/255.0, 232.0/255.0, 1.0);
    vec3 col = textureBlue;

    vec3 normal = getWaterNormal(pos * 4.0, time);
    //return col * col * (0.3 + 3.0 * dot(normal, sunLig));
    ...
    shadow = min(1.0, (shadow) + smoothstep(13.0, 15.0, length(pos - (vec3(0.5) - vec3(8.0, 0.0, -13.0)))));
    ...
    col = shadow * (0.15 + .3 * 0.5) * col + (0.3 + 0.65) * fresnel * skyReflect;
    return textureBlue * 0.8 + 0.8*col * water * water * (.85 + 1.0 * dot(normal, sunLig));
}
...
vec3 render(in vec3 ro, in vec3 rd, in vec3 rdx, in vec3 rdy, float time) { 
    ...
    // noise (when set to non-metal)
        ...
        // return vec3(1.0);
        // color balls?
        return waterColor(pos, rd, vec3(0.9, 0.6, -0.9), time);
        return vec3(151.0/255.0, 232.0/255.0, 1.0);
        K = vec2(0.05, 1.0);
        material = 2.0*vec3(151.0/255.0, 232.0/255.0, 1.0);
    } else if (m < 51.) {
...
void main() {
    ...
    for (int n=0; n < AA; n++) { 
        ...
        float time = 2.5*(uTime + (1.0/4.0) * float(m * n * AA) / float(AA*AA));
#else
